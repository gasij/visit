...
void main() {
    float time = uTime;
    vec2 pos = vUv;
    float o = uChromaOffset;
    float r = texture2D(uTexture, pos + cos(time * 2. - time + pos.x) * o).r;
    float g = texture2D(uTexture, pos + tan(time * .5 + pos.x - time) * o).g;
    float b = texture2D(uTexture, pos - cos(time * 2. + time + pos.y) * o).b;
...
class CanvasTxt {
  ...
  render() {
    ...
    const firstLineAscent = (metrics as unknown as { actualBoundingBoxAscent: number }).actualBoundingBoxAscent ?? this.fontSize * 0.8;

    lines.forEach((line, i) => {
      const yPos = 20 + firstLineAscent + i * lineHeight;
...
class CanvAscii {
  ...
  constructor(options: CanvAsciiOptions, containerElem: HTMLElement, width: number, height: number) {
    ...
    this.waveIntensity = options.waveIntensity ?? 0.4;
    this.chromaOffset = options.chromaOffset ?? 0.006;
  ...
  setMesh() {
    ...
    this.material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uChromaOffset: { value: this.chromaOffset }
      }
    });
  ...
  setRenderer() {
    ...
    this.container.addEventListener('mousemove', this.onMouseMove as (e: MouseEvent) => void);
    this.container.addEventListener('touchmove', this.onMouseMove as (e: TouchEvent) => void);
  }
  ...
  clear() {
    this.scene.traverse((obj) => {
      if ((obj as THREE.Mesh).isMesh && typeof (obj as THREE.Mesh).material === 'object' && (obj as THREE.Mesh).material !== null) {
        ...
        if ((obj as THREE.Mesh).geometry?.dispose) (obj as THREE.Mesh).geometry.dispose();
      }
    });
    this.scene.clear();
  }
  ...
  dispose() {
    ...
    this.container.removeEventListener('mousemove', this.onMouseMove as (e: MouseEvent) => void);
    this.container.removeEventListener('touchmove', this.onMouseMove as (e: TouchEvent) => void);
    this.clear();
    if (this.renderer) {
  ...
  useEffect(() => {
    ...
    const createAndInit = async (container: HTMLElement, w: number, h: number) => {
      const instance = new CanvAscii(
        { text, asciiFontSize, textFontSize, textColor, planeBaseHeight, enableWaves, waveIntensity, chromaOffset },
        container,
        w,
        h
      );
